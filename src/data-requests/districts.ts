...
export async function getNewDistrictDeaths(): Promise<ResponseData<{ ags: string; deaths: number }[]>> {
	const response = await axios.get(
...
export async function getLastDistrictCasesHistory(
	days?: number,
	ags?: string
): Promise<ResponseData<{ ags: string; name: string; cases: number; date: Date }[]>> {
...
export async function getLastDistrictDeathsHistory(
	days?: number,
	ags?: string
): Promise<ResponseData<{ ags: string; name: string; deaths: number; date: Date }[]>> {
...
export async function getLastDistrictRecoveredHistory(
	days?: number,
	ags?: string
): Promise<ResponseData<{ ags: string; name: string; recovered: number; date: Date }[]>> {
 ...
 */
export async function getAllDistrictCasesHistory(
	days?: number,
	ags?: string
): Promise<ResponseData<{ ags: string; name: string; cases: number; date: Date }[]>> {
	let allCases = Array<{ ags: string; name: string; cases: number; date: Date }>();
	let baseCases = 0;
	let casesSinceDay: ResponseData<Array<{
		...
		cases: number;
		date: Date;
	}>> = await this.getLastDistrictCasesHistory(days, ags);

	if (days != null) {
		let prevDaysResponse: any[] = await this.getAllDistrictCasesFromDay(days + 1, ags);
		baseCases = prevDaysResponse[0].cases;
	}

	for (let i = 0; i < casesSinceDay.data.length; i++) {
		allCases.push({
			ags: casesSinceDay.data[i].ags,
 ...
 */
export async function getAllDistrictCasesFromDay(day?: number, ags?: string): Promise<{ cases: number; date: Date }[]> {
	const whereParams = ['NeuerFall IN(1,0)'];
