...
export async function DistrictsCasesHistoryResponse(
	...
	for (const historyData of statesHistoryData.data) {
		...
		if (data[historyData.ags].history.length > 0) {
			...
			while (
				getDayDifference(nextDate, data[historyData.ags].history[data[historyData.ags].history.length - 1].date) > 1
			) {
...
interface DistrictsAllCasesHistory {
	[key: string]: DistrictHistory<{ cases: number; date: Date }>;
}
export async function DistrictsAllCasesHistoryResponse(
	days?: number,
	ags?: string
): Promise<DistrictsHistoryData<DistrictsAllCasesHistory>> {
	if (days != null && isNaN(days)) {
		throw new TypeError("Wrong format for ':days' parameter! This is not a number.");
	}
	...
	const statesHistoryData = await getAllDistrictCasesHistory(days, ags);

	const data: DistrictsAllCasesHistory = {};
