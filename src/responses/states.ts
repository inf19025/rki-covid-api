...
interface StatesAllCasesHistory {
	[key: string]: StateHistory<{ cases: number; date: Date }>;
}
export async function StatesAllCasesHistoryResponse(
	days?: number,
	abbreviation?: string
): Promise<StatesHistoryData<StatesAllCasesHistory>> {
	if (days != null && isNaN(days)) {
		throw new TypeError("Wrong format for ':days' parameter! This is not a number.");
	}
	...
	const statesHistoryData = await getAllStateCasesHistory(days, id);

	const data: StatesAllCasesHistory = {};
